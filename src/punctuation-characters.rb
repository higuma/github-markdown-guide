...
NAVIGATION = '[補足 - 名前付き文字参照一覧](named-character-references.md)
← [目次](index.md)'
SEPARATOR = '-' * 72
...
def output_line(f, base, items)
  ...
  for i in 0..15
    ...
    if items[code]
      ...
      f.print " <span id=\"#{hex}\" title=\"U+#{hex} (区切り文字ではない)\">-</span> |"
    end
  end
  f.puts
end
...
def output_markdown(f)
  ...
  f.puts SEPARATOR
  f.puts
  f.puts '[斜体]及び[太字]の検出判定に用いられる[区切り文字]の一覧。具体的には次のどちらかの条件に当てはまる文字が該当する。'
  f.puts
  f.puts '- [ASCII区切り文字]'
  ...
  f.puts NAVIGATION
  f.puts
  f.puts '[ASCII区切り文字]: characters.md#ASCII区切り文字'
  f.puts '[区切り文字]: characters.md#区切り文字'
