...
> &#x26A0;&#xFE0F; **日本語を扱う場合は非推奨**: これを用いると日本語には不要な無駄スペースが入る。また日本向けWebサイトではソフト改行を採用せず、改行に対して通常のテキスト文章と同じように改行(`<br>`)を挿入する仕様が多い(特に[Qiita])。文書の互換性を考慮するなら使うべきではない。
...
> 
> このように[空行]の挿入は可搬性の観点で大きな効果がある。[空行]不要なMarkdown実装は単に[空行]を無視するだけで無害なので、今は不要でも[空行]を入れておいた方が後々のトラブル防止に役立つ(特に他環境にコピーペーストする場合)。
> </details>
