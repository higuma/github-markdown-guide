...
これらは[ノーブレークスペース]を使って対応できる。まずコードスパン直前及びコードスパン先頭の空白有無に関する全ケースの振る舞い一覧を示す。両方に空白を確保する場合は直前空白として`&nbsp;`を用いるとよい。
...
次にコードスパン末尾及びコードスパン直後の空白有無の全ケースでの振る舞いは次の通り。この場合も両方に空白を確保する場合は直後空白に`&nbsp;`を用いればよい。
...
最後に文頭にあるコードスパンの先頭に空白を入れる場合は`&NoBreak;`を用いる。

```markdown
` EFG` ← コードスパン先頭の空白は除去

&NoBreak;` EFG` ← コードスパン先頭の空白は保存
```
...
ただしUnicode文字を直接入力することはできるので、これを利用してコードスパン中に縮約されない空白の連続を出力できる。次の例は空白部分にスペース(U+0020)ではなくノーブレークスペース(U+00A0)を直接入力して作成したもの。
