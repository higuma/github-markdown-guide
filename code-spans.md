...
| ---------------- | -------- | ----------------- | ----------- |
| 文頭             | 除去     | `` ` code` ``     | ` code`     |
| 空白             | 除去     | `` LMN ` code` `` | LMN ` code` |
| 非空白           | 保存     | `` LMN` code` ``  | LMN` code`  |
...
これらは[ノーブレークスペース]を使って対応できる。全ケースの一覧で示す。両方ありの場合に`&nbsp;`を用いると

| 直前空白 | 先頭空白 | 直前空白処理 | 先頭空白処理 |          記述          |       出力       |
| -------- | -------- | ------------ | ------------ | ---------------------- | ---------------- |
| なし     | なし     | -            | -            | `` LMN`code` ``        | LMN`code`        |
| なし     | あり     | -            | 保存         | `` LMN` code` ``       | LMN` code`       |
| スペース | なし     | 保存         | -            | `` LMN `code` ``       | LMN `code`       |
| スペース | あり     | 保存         | 除去         | `` LMN ` code` ``      | LMN ` code`      |
| `&nbsp;` | あり     | 保存         | 保存         | `` LMN&nbsp;` code` `` | LMN&nbsp;` code` |
...
コードスパンの直前や直後に`&nbsp;`を挿入すればよい。次のコード例では出力結果のコードスパン内外両方に空白が挿入されていることに注目すること。またコードスパン内部は先頭と末尾にスペース2個を用いている(処理時に1対削除されて1個ずつ残る)。
