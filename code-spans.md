...
| ---------------- | -------- | ----------------- | ----------- |
| 文頭             | 除去     | `` ` code` ``     | ` code`     |
| 空白の後         | 除去     | `` LMN ` code` `` | LMN ` code` |
| 非空白の後       | 保存     | `` LMN` code` ``  | LMN` code`  |
...
これらは[ノーブレークスペース]を使って対応できる。コードスパンの直前や直後に`&nbsp;`を挿入すればよい。次のコード例では出力結果のコードスパン内外両方に空白が挿入されていることに注目すること。またコードスパン内部は先頭と末尾にスペース2個を用いている(処理時に1対削除されて1個ずつ残る)。
