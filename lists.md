...
リスト項目は「**行頭マーカー** **スペース区切り** **コンテンツ**」の順で記述する。行頭マーカーは順序なしと順序ありで異なる書式を用いる。スペース区切りとコンテンツは両者で共通の書式を用いる。
...
先頭項目は順序数を設定できる。また2番目以降は0以上の整数なら何でも同じで、常に直前の項目に1を加えた値で表示する。
...
> <details>
> <summary>&#x26A0;&#xFE0F; <strong>実装者向け</strong>: スペース区切り</summary>
> 
> > &#x1F6AB; **文書作成法の習得が目的の読者には非推奨。常にスペース1個を入れると覚えておけばよい。**
> 
> > &#x2714;&#xFE0F; 以下の文例ではマーカーの種類を交互に変えていることに注意。2つの連続するリストの場合、[空行]だけではブロック区切りと認識されないためこの方法を用いている。
> 
...
> <details>
> <summary>&#x26A0;&#xFE0F; <strong>実装者向け</strong>: 行頭インデント</summary>
> 
> > &#x1F6AB; **文書作成法の習得が目的の読者には非推奨。これを使うと次で説明する[複数行に渡るリスト項目]との区別が難しくなり、実装によって出力が異なる状況が頻繁に発生する。**
> 
...
> &#x2714;&#xFE0F; **参考**: [Qiita]のMarkdownはこれとは異なる仕様で、スペースではなく改行を挿入し、テキストで見た通りに出力する。
...
この他にも任意のアイテムを再帰的に含むことができる。最後にやや技巧的なコードを示す。リストの先頭項目のコンテンツとして[コードブロック]、第2項目は[ブロック引用]からなり、さらに第2項目の[ブロック引用]はその内部に[タスクリスト]を含んでいる。
...
> &#x26A0;&#xFE0F; **注意**: 以下最後まで文書作成者ではなく実装者向けの詳細仕様の内容で、文書作成技術の習得が目的の読者は読む必要なし。標準的な記法から外れるほど実装に対する依存性が高くなり、意図しない動作を発生しやすい(上記の標準的記法以外は**全て非推奨**)。実装に興味がある読者向け。
...
> &#x2714;&#xFE0F; GitHub以外のMarkdownで仕様が異なる場合が多く、例えば[Qiita]では行連結の際にスペースではなく改行を挿入する。

### インデント

> &#x26A0;&#xFE0F; **注意**: 実装者向け。文書作成には不要な知識なので興味のない人は読む必要なし。
...
> &#x26A0;&#xFE0F; 解説はここまで。実装者に必要な詳細仕様は他にもまだ書き足りない程あるが、これ以上はもう本解説の範囲を超えている。また仕様書に書き切れない詳細仕様には実装者本人に委ねられている部分がどうしても存在し、そのような部分は実装者により動作が異なるためとても説明し切れない。
