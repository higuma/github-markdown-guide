...
> この条件によりスネークケース(e.g. `abc_def_ghi`)の中間にある`_`は強調とは認識されない。
...
7. 1,2のルールにより太字を適用すると各々の始端・終端の $N$ を2減らし、まだ1以上であればマーカーとして残して最初に戻り他の強調を再評価
8. 3,4のルールにより斜体を適用すると各々の始端・終端の $N$ を1減らし、まだ1以上であればマーカーとして残して最初に戻り他の強調を再評価
...
> 
> > ただし通常の環境では太字や斜体を多重掛けしても視覚効果は同じで、外見上では認識できない。CSSで特殊な設定をすれば話は別だが、GitHubではMarkdownからはCSS設定は行えないように保安上の対策がなされている(できない)。
...
次は5のオーバーラップの例。これは[ネスト](#ネスト)の最後の文例と同じものだがもう一度示す。

```markdown
**bold _bold+italic** italic_
```

構造は $a_L^2$ `bold ` $u_L^1$ `bold+italic` $a_R^2$ ` italic` $u_R^1$ となり、 $a_L^2$ **...** $a_R^2$ による太字と $u_L^1$ **...** $u_R^1$ が交差した状態になっている。この場合に先着優先を適用して先に検出した太字のみ適用し、斜体は適用せずそのままテキスト出力する(→ **bold _bold+italic** italic_)。
...
3. マッチした $a_L$ , $a_R$ の $N$ を1減らし、0になったら消去
...
> やれやれ、やっと全部解析したぞ。
> 
> いやまだひとつ残っている。[GFM仕様書6.4](https://github.github.com/gfm/#emphasis-and-strong-emphasis)のRules 9,10の意味が分からない。「3の倍数」って何？
