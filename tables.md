...
「行頭と行末に(もし不要でもマナーとして)`|`を入れる」と覚えておけばこのような間違いを未然に防ぐことができる(強く推奨)。
...
[空行]を入れても解決しない。(表を整形したのが仇となり)リストアイテム内のインデントと認識される。
...
> [コンテナブロック]の方が[葉ブロック]より構文解析の優先度が高い。表は[葉ブロック]扱いのため、表の構文解析中に[コンテナブロック]の文字列パターンを検出するとそこで終了と判定する。
...
> こういうことをいちいち覚えるよりも、より実用的・実践的に「表の後には[空行]が必要」とだけ覚えておくとよい。
