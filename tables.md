...
GitHubでは以前より独自のMarkdown構文を用いた表組みの機能が実装されていた。その後他のMarkdown実装にも採用され、現在では多くのMarkdown環境で利用可能な機能になっている。本章は[GitHub Markdown環境]をリファレンスとし、その仕様と動作について詳しく解説する。
...
> &#x2757;&#xFE0F; **注意**: 以下の内容は[GitHub Markdown環境]で実環境確認した内容であり、他のMarkdown環境では動作が異なる場合が多い点に留意すること。
...
> &#x2714;&#xFE0F; この場合は区切り行の先頭セルは`-`が2個以上必要。`- | -`ではGitHub実装は行頭の`- `を先に検出して[リスト項目]と判定する。ただし他のMarkdown処理実装には表と認識するものも多い(→ [Babelmark]を用いた[比較検証結果](https://babelmark.github.io/?text=A+%7C+B%0A-+%7C+-%0AA1+%7C+B1))。
