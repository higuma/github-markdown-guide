...
GitHubでは以前より独自のMarkdown構文を用いた表組みの機能が実装されていた。その後他のMarkdown実装にも採用され、現在では多くのMarkdown環境で利用可能な機能になっている。本章はGitHubの実環境を対象とし、Markdownによる表組み機能の仕様と動作について詳しく解説する。
...
> &#x2757;&#xFE0F; **注意**: 以下の内容はGitHubの実環境で確認した内容であり、他のMarkdown環境では動作が異なる場合が多い点に留意すること。
...
> &#x2714;&#xFE0F; ここから先は記法に関する考察が主な内容。細かい仕様及び実装の動作確認が中心で、ドキュメント作成記述の習得に専念したい人は読まなくてもよい。

### 行頭・行末の`|`

> &#x2714;&#xFE0F; 「行頭と行末の`|`は状況に応じて省略可能だが常に記述すべき」という趣旨。省略は可能だが、省略すると他の文法に誤認識されるケースがいくつも発生してしまう。以下はその退屈な説明だが、単に「常に記述する」と覚えておけば誤認識はまず発生せず、以下の内容を覚える必要もない。
...
> &#x2714;&#xFE0F; この場合は区切り行の先頭セルは`-`が2個以上必要。`- | -`ではGitHub実装は行頭の`- `を先に検出して[リスト項目]と判定する。ただし他のMarkdown処理実装には表と認識するものも多い(→ [Babelmarkによる検証結果](https://babelmark.github.io/?text=A+%7C+B%0A-+%7C+-%0AA1+%7C+B1))。
...
> &#x2714;&#xFE0F; この後も本章の最後まで同じような内容が続く。行頭・行末の`|`を省略せず、前後に空白を入れ、表の前後に空行を必ず入れておくことさえ覚えておけば読む必要なし。
