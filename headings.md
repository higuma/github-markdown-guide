...
> ここから先はMarkdownの成立と発展の過程に興味がある人向けの補足解説。初学者は読まずに先に進んでよいし、純粋にドキュメント作成のみが目的の人には不要な知識なのでやはり読む必要はない。
...
> 例えば下線行に`---`を用いる場合、それがSetext見出しの下線か、それとも[横線]かという判定の問題を生じる。それより**誤認識されない書き方**に専念すべきで、結論として**Setext見出しは使わない**のが最も現実的な選択になる(Markdown全般の解説なので仕方なく説明しているが、これから学ぶ読者には不要な知識)。
...
これもスペースさえ入れておけば全てのMarkdown実装が見出しと認識する。**スペースは必ず入れる**とだけ覚えておけば十分で、問題を未然に防ぐことができる。
