...
なおタイトル部の`<summary>`の内部にはMarkdown構文が効かない(GitHub環境特有の仕様)。この内部はHTMLで直接記述する必要がある。
...
ただし上記の方法では何重にネストしても全て同じレベルのように表示されるため、折りたたみをどのレベルまで展開した状態なのか読者に分かりにくい。その場合は[ブロック引用]と組み合わせて用いるとよい。
...
> &#x2714;&#xFE0F; **用語の日本語訳について**: 英語原文の"definition lists"に対する一般的な訳語は「定義リスト」「説明リスト」の2種類ある。訳語としては「定義リスト」の方が正確だが、日本版MDNではより平易な表現の「説明リスト」を用いており、本解説もMDNに従い「説明リスト」とした。
