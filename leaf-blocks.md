...
そのため前後に[空行]を挿入すれば間違いがない(またGFM以外のMarkdown処理系では[空行]が必要なものもある)。不要な[空行]は単に無視される(無害でより安全)。
...
* 前後に[空行]を入れる([Setext heading]などへの誤認識を防止)
...
* コンテンツの後に1個以上の空白を隔てて任意個の`#`、任意個の空白があってよい(除去される)
...
先頭に3つまで空白を挿入できる。4つ以上は[Indented code block]と認識される。
...
Markdownビューアを併用して記述作業を行う場合は次のようなミニマル仕様がお勧め。余計なものは一切付けないが、誤認識を未然に防止するため前後の[空行]は入れる。

* 先頭にヘッダレベルと同じ個数の`#`
* その後に空白1つ置いてからコンテンツ(任意の[インライン]を記述可能)
* コンテンツの末尾が`/ #+$/`の場合は`\`でエスケープ
* 行頭インデントなし、また行末に余分な空白や`#`は使わない
* 前後に[空行]を入れる(その方が安全、ないと誤認識する場合がある)
