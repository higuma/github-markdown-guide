...
* 先頭に0-3文字の空白(4文字以上は[Indented code block]として認識される)
* `-`が3文字以上、または``_``が3文字以上、または``*``が3文字以上
    * 全て同じ文字(行中に空白以外の別の文字があってはならない)
    * 中間に任意個の空白があってもよい
* その後に空白があってもよい
...
ただし区切り文字に`-`を使うと[Setext heading]と認識されることに注意。
...
これは失敗例で、次のように`Foo`が``<h2>``に変換される。

> Foo
> ---
> bar

そのため前後に空行を挿入すれば間違いがない(またGFM以外のMarkdown処理系では空行が必要なものもある)。不要な空行は単に無視される(無害でより安全)。

### Thematic breaks: Best practice

表現方法に幅がありすぎて、人によってまるで違う書き方になることが多い。何でもいいから自分流の書き方を決めてそれで通すとよい。以下はその一例で、余計なものを一切入れず、さらにGFM以外のMarkdownでも誤認識しないように配慮している。

* `--------------------`のように十分な長さのハイフンを用いる(空白なし)
* 前後に空行を入れる([Setext heading]などへの誤認識を防止)
...
* 行の先頭文字は`#`(→``<h1>``), `##`(→``<h2>``), ... , `######`(→``<h6>``)
    * 手前に3文字まで空白を挿入可能
* 直後に1個以上の空白の後にコンテンツ(見出し文)
    * コンテンツ中に任意の[インライン]を挿入できる
* コンテンツの後に空白を置き、その後に任意個の`#`、任意個の空白があってよい(無視される)
...
HTMLの見出し要素は``<h6>``までしかないので、7つ以上はテキストとして扱われる。
...
`#`, `##`, `###`, ...とコンテンツの間には空白が必要(ないとテキストとして扱われる)。なお2つ以上空白を挿入しても縮約されて同じ出力になる。
...
ATX headingの形式に一致する行を通常テキストとして扱う場合は`\`でエスケープする。
...
先頭に3つまで空白を挿入できる。4つ以上は[Indented code block](#44-indented-code-blocks)と認識される。
...
行末に1個以上の空白、任意個の`#`、任意個の空白([正規表現]では``/ +#* *$/``)があってよい。これらは単に無視される。また空白を挟まずに`#`で終了している場合はテキストとしてそのまま出力する。
...
逆に空白に続く末尾の`#`を文字として表示する場合は`\`でエスケープする。
...
* 前後に空行は不要、ただし他の処理系も考慮すると付けた方が安全(見やすさの効果もある)
* コンテンツは空でもよい

### ATX headings: Best practice

Markdownビューアを併用して記述作業を行う場合は次のようなミニマル仕様がお勧め。余計なものは一切付けないが、誤認識を未然に防止するため前後の空行は入れる。

* 先頭にヘッダレベルと同じ個数の`#`
* その後に空白1つ置いてからコンテンツ(任意のインラインを記述可能)
* コンテンツの末尾が`/ #+$/`の場合は`\`でエスケープ
* 行頭インデントなし、また行末に余分な空白や`#`は使わない
* 前後に空行を入れる(その方が安全、ないと誤認識する場合がある)
...
コンテンツテキストの次行に1つ以上の`=`(→ ``<h1>``)または1つ以上の`-`(→ ``<h2>``)を用いて見出しを表現できる。
...
* コンテンツは複数行にまたがっていてもよい(改行は空白に置換される)
* コンテンツは空であってはならない([ATX headings]と異なる)
* 見出し表示行は1つ以上の`=`(→``<h1>``)または1つ以上の`-`(→``<h2>``)
    * 行頭に3つまで空白を挿入可能
    * 行末に任意個の空白があってもよい
    * [正規表現]を用いると ``/^ {0,3}(?:=+|-+) *$/``
* 前後に空行は不要(だが経験上入れないと時々誤認識する)
...
行頭に4つ以上の連続した空白がある行はコードブロックとして扱われ、行頭の空白4つを除去した後、テキスト文書として見た通りに出力する。これは特に次のようなコードリスティングに用いられる。
...
ただし前の行が[リストアイテム](container-blocks.md#52-list-items)の場合はリストアイテムの続きと認識される。空行を挿入してもリストのインデントとして扱われる。
...
* コードフェンスは開始・終了とも手前に3つまで空白を挿入可能、また末尾の空白は無視
...
[info string]はコードブロックの書式設定用で、言語名を小文字で設定する。間と行末に空白があってもよい。
...
* [info string]は`` ```java ``のように言語名を小文字で記述(今は間に空白を入れない方が主流)
...
HTMLテキストをそのまま出力させる機能。仕様書上は全部で7通り用意しているが、サポートしていない処理系が多い(本家のGitHubですら...)。そのため説明は省略する。

> またネット上には膨大な数のMarkdown文書があるが、未だに使われているのを見たことがない。
...
後は記法のバリエーションについて補足説明する。まず[リンクラベル]の手前には3文字までのインデント空白が可能。[リンク先]の前には任意個の空白が可能(なくてもよい)。タイトルを設定する場合は手前に1個以上の空白が必要。また行末に空白があれば除去して処理する。
